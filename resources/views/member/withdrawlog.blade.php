...
@section('content')
    <header class="aui-bar aui-bar-nav">
        ...
        <a class="aui-pull-right aui-btn" href="{{route('member.withdraw')}}" >
    ...
    <section class="aui-content aui-card-list-content aui-margin-b-15">
        <ul class="aui-list aui-media-list aui-list-in" id="withdraw-content">
            @include('member.withdrawlogitem')
        </ul>
    </section>
...
<script>
    ...
    var scroll = new auiScroll({
        ...
        $.getJSON( '{{route('member.withdrawlog')}}' , { page : currentPage } , function( data ){
            loading = false
            if( data.errcode === 0 ) {
                $('#withdraw-content').append( data.data )
            }
        }) ;
    });
</script>
