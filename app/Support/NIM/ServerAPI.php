...
class ServerAPI
{
 ...
 // SHA1(AppSecret + Nonce + CurTime),三个参数拼接的字符串，进行SHA1哈希计算，转化成16进制字符(String，小写)
     ...
     * @param $type [0
     *            表示文本消息,1 表示图片，2 表示语音，3 表示视频，4 表示地理位置信息，6 表示文件，100 自定义消息类型]
     ...
     *            + payload不能超过200字节）]
     * @param $payload [ios
     *            推送对应的payload,必须是JSON（pushcontent + payload不能超过200字节）]
     ...
     * 消息功能-文件上传
     * 
     * @param $content [字节流base64串(Base64.encode(bytes))
     *            ，最大15M的字节流]
     * @param $type [上传文件类型]
     * @return $result [返回array数组对象]
     */
