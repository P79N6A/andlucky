...
class ClassMirrorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function it_reflects_return_typehints()
    {
        ...
        $this->assertEquals('string', $classNode->getMethod('getName')
            ->getReturnType());
        $this->assertEquals('\Fixtures\Prophecy\WithReturnTypehints', $classNode->getMethod('getSelf')
            ->getReturnType());
        $this->assertEquals('\Fixtures\Prophecy\EmptyClass', $classNode->getMethod('getParent')
            ->getReturnType());
    }
