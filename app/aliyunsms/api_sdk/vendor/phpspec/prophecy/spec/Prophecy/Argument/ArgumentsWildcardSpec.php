...
class ArgumentsWildcardSpec extends ObjectBehavior
{
    ...
    function it_exposes_list_of_tokens(TokenInterface $token)
    {
        ...
        $this->getTokens()->shouldReturn(array(
            $token
        ));
    }
    ...
    function it_returns_false_if_there_is_less_arguments_than_tokens(TokenInterface $token1, TokenInterface $token2, TokenInterface $token3)
    {
        $token1->scoreArgument('one')->willReturn(3);
        $token1->isLast()->willReturn(false);
        $token2->scoreArgument(2)->willReturn(5);
    ...
    function it_returns_false_if_there_is_less_tokens_than_arguments(TokenInterface $token1, TokenInterface $token2, TokenInterface $token3)
    {
        $token1->scoreArgument('one')->willReturn(3);
        $token1->isLast()->willReturn(false);
        $token2->scoreArgument(2)->willReturn(5);
    ...
    function it_should_return_false_if_one_of_the_tokens_returns_false(TokenInterface $token1, TokenInterface $token2, TokenInterface $token3)
    {
        $token1->scoreArgument('one')->willReturn(3);
        $token1->isLast()->willReturn(false);
